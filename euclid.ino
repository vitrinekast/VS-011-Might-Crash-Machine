...
int* getEuclid(int* pattern, int length, int k, int offset)
{
...
void euclid(int length, int k, int chan, int offset)
{
  Serial.println("euclid?");
   getEuclid(euclid_pattern[chan], length, k, offset);
  int digitCounter = 0;
...
void setDrumEuclid()
{
  ...
  // loop trough every layer to set the sequence to the current phase
  for (int i = 0; i < DRUM_SEQ_LAYERS; i++)
  {
    getEuclid(drum_eucl_sequence[i], 16, drum_eucl_structure[i][song_state], 0);
  }

  Serial.println("just set all the new drum sequence stuff");

  for (int i = 0; i < DRUM_SEQ_LAYERS; i++)
  {
    Serial.print("Layer: ");
    Serial.print(DRUM_SEQ_LAYERS);


    for(int index = 0; index < 16; index++) {
      Serial.print(drum_eucl_sequence[i][index]);
    }
  }
}
